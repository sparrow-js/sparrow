import {
  ...
  import { saveflowInfo } from '../../api';
  import { serialNode } from '../components/ChatMessage/serialNode';
  ...
  export function TabsProvider({ children }: { children: ReactNode }) {
    ...
    function nodeToVetorstores(node: any) {
      const { data } = node;
      ...
      if (answer.type === 'list') {
        metadata = {};
        answer.dataList.forEach((item: any) => {
      ...
      const param: any = {
        pageContent: question.value,
        metadata,
      };

      if (link) {
        param.child = link.reduce((prev: any[], item: any) => {
          prev.push(nodeToVetorstores(item));
          return prev;
        }, []);
      }
    ...
    async function saveChain() {
      let saveflows = _.cloneDeep(flows);
      const nodes = serialNode(saveflows[0]);
      const headNode = nodes[0];
      const vetorsParam = nodeToVetorstores(headNode);
      vetorsParam.metadata.flow = saveflows;
      console.log('**************vetors', vetorsParam);
      return await saveflowInfo(vetorsParam);
    }
