  ...
  function handleSearchInput(e: string) {
    setFilterData((_) => {
      ...
      Object.keys(data).forEach((d: keyof APIObjectType, i) => {
        ret[d] = {};
        let keys = Object.keys(data[d]).filter((nd) => nd.toLowerCase().includes(e.toLowerCase()));
    ...
    <>
      ...
      <div className="mt-1 w-full">
        {Object.keys(dataFilter)
          .sort()
          .map((d: keyof APIObjectType, i) => (Object.keys(dataFilter[d]).length > 0 ? (
            <DisclosureComponent
              openDisc
              key={i}
              button={{
                  title: nodeNames[d],
                  Icon: nodeIcons.unknown,
                }}
            >
