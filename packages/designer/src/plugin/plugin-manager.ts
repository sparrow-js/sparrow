import { Editor, engineConfig } from '@firefly/auto-editor-core';
...
export class AutoCodePluginManager implements ILowCodePluginManager {
    ...
    async delete(pluginName: string): Promise<boolean> {
        const idx = this.plugins.findIndex((plugin: any) => plugin.name === pluginName);
